...
(async()=>{
  ...
  try {
    ...
    let r=await request(`http://127.0.0.1:${appPort}/health`); if(r.status!==200||!r.body.moveworksConfigured||r.body.version!=='10.3.0') throw new Error('health failed');
    ...
    r=await request(`http://127.0.0.1:${appPort}/api/dashboard`); if(r.status!==200||r.body.sla?.breached!==3||r.body.sla?.totalAttention!==8||r.body.ageing?.total!==9||r.body.aiBriefing!=='Callback AI analysis') throw new Error('callback dashboard projection failed');
    ...
    const page=await fetch(`http://127.0.0.1:${appPort}/`).then(r=>r.text()); if(!page.includes('AI Operations Agent')) throw new Error('page branding failed');
    const appJs=await fetch(`http://127.0.0.1:${appPort}/app.js`).then(r=>r.text()); if(!appJs.includes('AI Operations Agent')||!appJs.includes('startVoice')||!appJs.includes('renderResults')||!appJs.includes('speakVoiceResponse')) throw new Error('MVP agent UI missing');
