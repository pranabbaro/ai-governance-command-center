...
(async()=>{
  ...
  try {
    ...
    let r=await request(`http://127.0.0.1:${appPort}/health`); if(r.status!==200||!r.body.moveworksConfigured||r.body.version!=='12.7.0') throw new Error('health failed');
    ...
    const page=await fetch(`http://127.0.0.1:${appPort}/`).then(r=>r.text()); if(!page.includes('Moveworks Hackathon')||!page.includes('app.js?v=12.7.0')) throw new Error('page branding/cache-bust failed');
