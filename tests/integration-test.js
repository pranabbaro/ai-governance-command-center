...
(async()=>{
  ...
  try {
    ...
    r=await request(`http://127.0.0.1:${appPort}/api/ai/query`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:'Give me RCA for INC5784096'})}); if(r.status!==202||r.body.mode!=='webhook-trigger'||!r.body.requestId||r.body.request_id!==r.body.requestId||r.body.started_at!==r.body.startedAt||r.body.incident_number!==r.body.incidentNumber) throw new Error('RCA webhook trigger/correlation aliases failed');
