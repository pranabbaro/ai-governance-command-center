...
(async()=>{
  ...
  try {
    ...
    let r=await request(`http://127.0.0.1:${appPort}/health`); if(r.status!==200||!r.body.moveworksConfigured||r.body.version!=='12.5.0') throw new Error('health failed');
    ...
    if(r.status!==200||r.body.mode!=='synchronous-rca'||r.body.incident_number!=='INC5784096'||r.body.answer!=='Direct incident-specific RCA text') throw new Error('Direct synchronous RCA failed');
    if(!r.body.requestId||r.body.request_id!==r.body.requestId) throw new Error('Direct RCA response metadata failed');
    r=await request(`http://127.0.0.1:${appPort}/api/dashboard`);
    if(r.status!==200||r.body.sla?.breached!==3) throw new Error('Direct RCA changed governance dashboard');
    ...
    const page=await fetch(`http://127.0.0.1:${appPort}/`).then(r=>r.text()); if(!page.includes('Moveworks Hackathon')||!page.includes('app.js?v=12.5.0')) throw new Error('page branding/cache-bust failed');
